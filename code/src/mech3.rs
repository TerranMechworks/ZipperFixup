...
extern "fastcall" fn draw_line(screen_buffer: *mut u16, x1: i32, y1: i32, x2: i32, y2: i32, color: i16){
...
extern "fastcall" fn draw_dashed_line(screen_buffer: *mut u16, x1: i32, y1: i32, x2: i32, y2: i32, color: i16, sections: i32){
    let (x1_out, y1_out) = clamp_to_screen_space(x1,y1);
    let (x2_out, y2_out) = clamp_to_screen_space(x2,y2);

    const DRAW_DASHED_LINE_ADDR: *const() = 0x00563c50 as *const();
    let func = unsafe{addr_as_fn!(DRAW_DASHED_LINE_ADDR, DrawDashedLineFn)};
    func(screen_buffer, x1_out, y1_out, x2_out, y2_out, color, sections);
}